...
def LyapunovOpt(task,N):
    ...
    if min_pro < 0 or accessGPU == task.maxAccessGPU() + 1 or accessGPU == 9:
        if min_pro >= 0:
...
def LyapunovOptstart():
    ...
    while(len(All_task) < 200 ):
        ...
        #调度当前队首任务是否可以放入（调度核心区域）
        if taskQueue.showLen() > 0 and num > 0:
            LyapunovOpt(taskQueue.task_list[0],num)
        elif taskQueue.showLen() > 0 and num == 0:
            virtualQueue.addLen(0)
        else:
            virtualQueue.deleteLen()
...
def countUsingTime():
    ...
    np.save("Lyp_wait.npy",waitTimeList)
    np.save("Lyp_using.npy", usingTimeList)
